...
<body>
    <form class="card" action="MemberJoinProc.jsp" method="post" autocomplete="off">
      ...
      <div class="row">
        ...
        <div class="input-group">
          <input class="input" type="text" id="email" name="email" placeholder="이메일을 입력하세요" required>
          <button class="subbtn" type="button" onclick="checkEmail()">중복확인</button>
        </div>
        <span id="emailResult" class="hint"></span>
      </div>
  ...
  <script>
    ...
    function checkEmail() {
        ...
        if (!id) { alert("이메일을 입력해주세요!"); return; }
        const pattern = /^[0-9a-zA-Z]([-_.]?[0-9a-zA-Z])*@[0-9a-zA-Z]([-_.]?[0-9a-zA-Z])*\.[a-zA-Z]{2,3}$/i;
        if (!pattern.test(email)) { alert("이메일 형식을 지켜주세요!"); return; }

        fetch("EmailCheck.jsp?email=" + encodeURIComponent(email))
          .then(r => r.text())
          .then(t => resultSpan.innerHTML = t)
          .catch(e => console.error(e));
      }
