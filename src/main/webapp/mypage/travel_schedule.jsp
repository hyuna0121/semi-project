...
// 🚨🚨🚨 특수 문자 이스케이프 헬퍼 함수 (JSON 안전성 확보) 🚨🚨🚨
java.util.function.Function<String, String> escapeJson = (str) -> {
    if (str == null) return "";
    return str.replace("\\", "\\\\")
              .replace("'", "\\'") 
              .replace("\r", "")
...
// 🔸 캘린더 JSON 데이터 생성 - 1. 내가 만든 일정
   ...
   jsonEvents.append(",");
      ...
      String calendarEndDate = null;
// FullCalendar에 전달할 +1된 종료일
...
// +1된 날짜 포맷
      ...
      // 🚨🚨🚨 [수정]: escapeJson 함수를 사용하여 title/description/location 이스케이프 처리 🚨🚨🚨
      jsonEvents.append("{");
      jsonEvents.append("title: '").append(escapeJson.apply(schedule.getTitle())).append("',");
...
jsonEvents.append("location: '").append(schedule.getLocation() != null ? escapeJson.apply(schedule.getLocation()) : "")
      .append("',");
jsonEvents.append("description: '").append(schedule.getDescription() != null ? escapeJson.apply(schedule.getDescription()) : "")
      .append("',");
...
Calendar c = Calendar.getInstance();
      ...
      // 🚨🚨🚨 [수정]: escapeJson 함수를 사용하여 title/description/location 이스케이프 처리 🚨🚨🚨
      jsonEvents.append("{");
      jsonEvents.append("title: '").append(escapeJson.apply(schedule.getTitle())).append(" (참여)',");
      jsonEvents.append("start: '").append(startDate).append("',");
if (calendarEndDate != null) { // ✅ 유효한 +1된 종료일이 있을 경우에만 추가
   jsonEvents.append("end: '").append(calendarEndDate).append("',");
}
                     ...
                     <tbody>
                        ...
                        %>
                         <tr data-schedule-id="<%=schedule.getId()%>"
                           ...
                           data-location="<%=schedule.getLocation() != null ?
schedule.getLocation() : ""%>"
                           data-desc="<%=schedule.getDescription() != null ?
schedule.getDescription() : ""%>">
                           ...
                           <td><%=schedule.getCreatedAt() != null ? schedule.getCreatedAt() : "-"%></td>
   ...
   <script src="js/travel_schedule.js"></script>
    ...
    <div class="modal fade" id="eventModal" tabindex="-1"
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg"> <div class="modal-content">
                <div class="modal-header bg-info text-white"> <h5 class="modal-title" id="eventModalLabel">일정 상세보기</h5>
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-6 border-end">
                            <h4 class="mb-3" id="modalTitle">제목</h4>

                            <p class="mb-1"><strong>시작일:</strong> <span id="modalStart"></span></p>
                            <p class="mb-1"><strong>종료일:</strong> <span id="modalEnd"></span></p>
                            <p class="mb-1"><strong>지역:</strong> <span id="modalLocation"></span></p>
                            
                            <hr class="mt-3"/>
                            <p class="mt-3">
                                <strong>설명:</strong> 
                                <span id="modalDesc" class="d-block mt-1 p-2 bg-light rounded small"></span>
                            </p>
                        </div>

                        <div class="col-md-6">
                            <h5 class="mb-3 border-bottom pb-2">세부 일정</h5>
                            
                            <div id="detailedScheduleArea">
                                <ul class="nav nav-tabs" id="scheduleTabs" role="tablist">
                                </ul>
                                <div class="tab-content" id="scheduleTabContent" style="max-height: 300px; overflow-y: auto;">
                                    <p class="text-center text-muted mt-3">세부 정보를 불러오는 중...</p>
                                </div>
                            </div>
                        </div>
                    </div> 
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-primary" id="goToDetailsBtn"
                        data-schedule-id="">
                        <span class="material-symbols-outlined align-middle me-1">calendar_month</span>
                        상세 일정 보기
                    </button>
                </div>
            </div>
