...
// 작은따옴표(')를 \'로, 백슬래시(\)를 \\로, 줄 바꿈을 \\n으로 변환하여 JavaScript 구문 오류를 방지
java.util.function.Function<String, String> escapeJson = (str) -> {
	if (str == null)
		return "";
	return str.replace("\\", "\\\\") // 백슬래시 자체를 이스케이프
	.replace("'", "\\'") // 작은따옴표를 이스케이프 (가장 흔한 에러 원인)
	.replace("\r", "") // 캐리지 리턴 제거
	.replace("\n", "\\n"); // 줄바꿈을 이스케이프
};
...
try {
	...
	try {
		...
		// 날짜 형식 오류 발생 시 (e.g. DB 포맷 불일치), 오류를 기록하고 end 속성 추가를 건너
	...
	jsonEvents.append("end: '").append(calendarEndDate).append("',");
		...
		jsonEvents.append("location: '")
		.append(schedule.getLocation() != null ? escapeJson.apply(schedule.getLocation()) : "").append("',");
		jsonEvents.append("description: '")
		.append(schedule.getDescription() != null ? escapeJson.apply(schedule.getDescription()) : "")
		.append("',");
...
<body>
   ...
   <%=jsonEvents.toString()%>
      ...
      // 컨텍스트 경로를 전역 변수에 저장
      window.CTX = '<%=request.getContextPath()%>'; // 🚨 줄 바꿈, 공백 제거
	</script>
		...
		<div class="modal-dialog modal-dialog-centered">
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<hr />
					<h6 class="mt-3">세부 일정 목록</h6>
					<div id="detailedScheduleList">
						<p class="text-center text-muted">세부 정보를 불러오는 중...</p>
					</div>
				</div>
